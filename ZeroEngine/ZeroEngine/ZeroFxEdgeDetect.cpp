...
ZeroFxEdgeDetect::ZeroFxEdgeDetect(float _time, char* _renderPage) : ZeroIShader(_renderPage), time(_time), beginTime(_time) {
...
void ZeroFxEdgeDetect::Update(float _eTime) {
	...
	D3DXVECTOR4 pixelOffset = D3DXVECTOR4(1.0f / ZeroApp->GetWindowWidth(), 1.0f / ZeroApp->GetWindowHeight(), 0.0f, 0.0f);
	if (ZeroRTMgr(renderPage)->Shader())
