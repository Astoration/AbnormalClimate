	...
	if (followingTarget) {
		if (moveSpeed > 0) {
			currentPosition.x += 
				...
				(followingTarget->Width() * followingTarget->Scale().x) / 2) * _eTime * moveSpeed;
			currentPosition.y += 
				(followingTarget->Pos().y - currentPosition.y - (ZeroApp->GetWindowHeight() / 2.0f) + 
				(followingTarget->Height() * followingTarget->Scale().y) / 2) * _eTime * moveSpeed;
		} else {
			currentPosition.x = followingTarget->Pos().x + 
				(followingTarget->Width() * followingTarget->Scale().x) / 2 - (ZeroApp->GetWindowWidth() / 2.0f);
			currentPosition.y = followingTarget->Pos().y +
				(followingTarget->Width() * followingTarget->Scale().y) / 2 - (ZeroApp->GetWindowHeight() / 2.0f);
		}
	}
