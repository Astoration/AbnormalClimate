...
ZeroParticleSprite::ZeroParticleSprite(char* _path, int _size) {
	count = 0;
	time = 0.0f;
...
void ZeroParticleSprite::CreateParticle() {
	...
	ZeroVec speed = ZeroVec(randspeed * ZeroMathMgr->Cos(randspread),
							randspeed * ZeroMathMgr->Sin(randspread));
	float lifetime = ZeroMathMgr->RandFloat(minLifeTime, maxLifeTime);
	float size = ZeroMathMgr->RandFloat(minScale, maxScale);
	count++;

	ZeroIParticle *particle = new ZeroIParticle(path, speed, lifetime, size, rotSpeed, startA, startR, startG, startB, renderState);
...
void ZeroParticleSprite::Update(float _eTime) {
	...
	for (PARTICLE::iterator iter = particleList.begin(); iter != particleList.end(); iter++) {
		...
		if (static_cast<int>(startA + (*iter)->NowLifeTime() / (*iter)->LifeTime() * (endA - startA)) > 0) {
			(*iter)->SetColorA(static_cast<int>(startA + (*iter)->NowLifeTime() / (*iter)->LifeTime() * (endA - startA)));
		}
		else if (255 - endA != 0) {
			(*iter)->SetColorA(0);
		}

		(*iter)->SetColorR(static_cast<int>(startR + ((*iter)->NowLifeTime() / (*iter)->LifeTime() * (endR - startR))));
		(*iter)->SetColorG(static_cast<int>(startG + ((*iter)->NowLifeTime() / (*iter)->LifeTime() * (endG - startG))));
		(*iter)->SetColorB(static_cast<int>(startB + ((*iter)->NowLifeTime() / (*iter)->LifeTime() * (endB - startB))));
