...
ZeroTrCircleReveal::ZeroTrCircleReveal(float _time, char* _renderPage, LPDIRECT3DTEXTURE9 _secondTexture)
	: ZeroIShader(_renderPage)
	, time(_time)
	, beginTime(_time)
	, secondTexture(_secondTexture) {
	if (!ZeroRTMgr(renderPage)->Shader())
		ZeroRTMgr(renderPage)->SetShader("Texture/Transition/CircleReveal.fx");
	else
		SetErase(true);
}
ZeroTrCircleReveal::~ZeroTrCircleReveal() {
}

void ZeroTrCircleReveal::Update(float _eTime) {
	time -= _eTime;
