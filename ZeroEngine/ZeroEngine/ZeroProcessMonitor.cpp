...
// 생성자/소멸자 구현
ZeroProcessMonitor::ZeroProcessMonitor() : process(NULL), cpuUsageQuery(NULL), cpuUsageCounter(NULL) {
	Create();
}
...
HRESULT ZeroProcessMonitor::Create() {
	...
	if (0 == GetModuleBaseName(process, hModule, processName, sizeof(processName) / sizeof(TCHAR))) {
		// TODO: 에러 처리
		// GetLastError()
...
DWORD ZeroProcessMonitor::GetMemoryUsage() {
	// Memory 사용량 정보 처리
