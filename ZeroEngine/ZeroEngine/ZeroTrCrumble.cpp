...
ZeroTrCrumble::ZeroTrCrumble(float _time, char* _renderPage, LPDIRECT3DTEXTURE9 _secondTexture)
	...
	if (!ZeroRTMgr(renderPage)->Shader())
		ZeroRTMgr(renderPage)->SetShader("Texture/Transition/Crumble.fx");
	else
		SetErase(true);

	m_pCloudTex = ZeroTexMgr->LoadTextureFromFile("Texture/Transition/Cloud.png");
}
ZeroTrCrumble::~ZeroTrCrumble() {
}

void ZeroTrCrumble::Update(float _eTime) {
	time -= _eTime;
