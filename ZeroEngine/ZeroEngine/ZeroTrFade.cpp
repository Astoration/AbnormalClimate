...
ZeroTrFade::ZeroTrFade(float _time, char* _renderPage, LPDIRECT3DTEXTURE9 _secondTexture)
	: ZeroIShader(_renderPage)
	, time(_time)
	, beginTime(_time)
	, secondTexture(_secondTexture) {
	if (!ZeroRTMgr(renderPage)->Shader())
		ZeroRTMgr(renderPage)->SetShader("Texture/Transition/Fade.fx");
	else
		SetErase(true);
}
ZeroTrFade::~ZeroTrFade() {
}

void ZeroTrFade::Update(float _eTime) {
	time -= _eTime;
