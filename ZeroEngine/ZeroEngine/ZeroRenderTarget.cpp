...
ZeroRenderTarget::ZeroRenderTarget() {
	...
	D3DXCreateTexture(ZeroApp->GetDevice(), width, height, 1,
		D3DUSAGE_RENDERTARGET, D3DFMT_UNKNOWN, D3DPOOL_DEFAULT, &texture);
...
void ZeroRenderTarget::OnPrevDeviceReset() {
	HRESULT hr = D3DXCreateTexture(ZeroApp->GetDevice(), width, height, 1,
		D3DUSAGE_RENDERTARGET, D3DFMT_UNKNOWN, D3DPOOL_DEFAULT, &texture);
	texture->GetSurfaceLevel(0, &surface);
}
...
void ZeroRenderTarget::SetShader(char * _path) {
	if (shader == NULL) {
